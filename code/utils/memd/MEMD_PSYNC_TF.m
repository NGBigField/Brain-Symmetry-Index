...
function [RO,F_vec,T_vec,avai_pairs]=MEMD_PSYNC_TF(X,Fs,W,FR,E_Th,NFB,W_OV,IMF)

% Determines the phase synchrony between x1 and x2 using bivariate EMD

% Required functions : 'emd.m' by Rilling et al.

%-----------       OUTPUT     ------------------------------
% 'RO' is an M X N matrix with each entry corresponding to the 
...
%
% 'F_vec' is a 1 X M vector which denotes the frequency grid for 'RO'
%
% 'T_vec' is a 1 X N vector which denotes the time grid for 'RO'
%
% To dispay the results, one could enter the following:
% 'mesh([1:size(RO,2)],IF,RO);view(0,90);'
...
%      Suggest 200.
% FR :: The steps in normalised frequency that determine the resolution
%          of 'RO' and subsequently the size of 'M'.
%          Should be of the range 0 < FR <= .5
%          Suggestion 0.002  
% E_Th  :: Threshold for component relevance
%          Suggestion .2;
% NFB   :: number of freq. bins to be included in analysis (starting from 0Hz). 
%          cannot exceeed (.5/FR) 
% W_OV :: Number of samples for windows overlapping     Apit added 230414
...
%% Obtain IMFs
% imf=emd(x1+(1i*x2));  Apit commented out 240414
% [NUM_IMFs,len]=size(real(imf));   Apit commented out 240414

%%%%% Apit added  240414 %%%%%%%%
% IMF=memd(X,64,'fix_h',8);
% IMF=memd(X);
...
avai_pairs=nchoosek(1:num_ch,2);

% Number of pairs
num_pairs=size(avai_pairs,1);
...
RO=zeros(num_pairs,int32(NFB),q_count);    

for pair_count=1:num_pairs
    
    %% Determing minium index
    imf=zeros(2,size(IMF,2),size(IMF,3));
    imf(1,:,:)=squeeze(IMF(avai_pairs(pair_count,1),:,:));
    imf(2,:,:)=squeeze(IMF(avai_pairs(pair_count,2),:,:));

       
    x1=X(avai_pairs(pair_count,1),:);
    x2=X(avai_pairs(pair_count,2),:);
...
%         [indmin,indmax] = locate_extrema(imag(imf(k,:)));   Apit commented out 250414
    ...
    % MEM ALLOCATION
    % % RO=zeros(.5/FR,len-(W+1));
    % RO=zeros(NFB,len-(W+1));  Apit commented out 240414

    %% Phase synchrony calculation

    for nn=1:IDX
        % [ro,Ar,Ai,Fr,Fi,T_vec]=PHASE_SYNC_local(real(imf(nn,:)),imag(imf(nn,:)),Fs,W,W_OV);  Apit commented out 250414
        % IF_AVG=(Fr(:,W/2:end-(W/2+2))+Fi(:,W/2:end-(W/2+2)))/2;    Apit commented out 240414

        % Apit added 240414
        [ro,Ar,Ai,Fr,Fi,T_vec]=PHASE_SYNC_local(squeeze(imf(1,nn,:))',squeeze(imf(2,nn,:))',Fs,W,W_shift);
        IF_AVG=(Fr(:,W/2:W_shift:end-W/2+1)+Fi(:,W/2:W_shift:end-W/2+1))/2;

        IF_norm=IF_AVG/Fs;

        % Threshold phase sync. based standard deviation of i/p data --
        E1=sqrt(var(x1));
        E2=sqrt(var(x2));
        Ar_T=double(Ar>(E_Th*E1));
        Ai_T=double(Ai>(E_Th*E2));
        % ro_T=Ar_T(:,W/2:end-(W/2+2)).*Ai_T(:,W/2:end-(W/2+2)).*ro;   Apit commented out 240414
        
        % Apit added 240414
        ro_T=Ar_T(:,W/2:W_shift:end-W/2+1).*Ai_T(:,W/2:W_shift:end-W/2+1).*ro;

        %--------------------------------------------------------------
        [RO_temp,F_vec_norm]=HIL_HUANG_SPEC_local(IF_norm,ro_T,FR,NFB);
        
        RO(pair_count,:,:)=squeeze(RO(pair_count,:,:))+RO_temp;
